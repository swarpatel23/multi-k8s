...
kubectl apply -f k8s
kubectl set image deployments/server-deployments server=swar23/multi-server:$SHA
kubectl set image deployments/client-deployments client=swar23/multi-client:$SHA
kubectl set image deployments/worker-deployments worker=swar23/multi-worker:$SHA
